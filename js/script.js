...
generate.addEventListener("click", function(e) {
  e.preventDefault()
  generateText(parseInt(number.value))
})


let generateText = (num) => {
  //empty
  // <0
  // >9

  if (isNaN(num) || num<=0 || num>9) {
    paragraph.innerHTML = `<p class="textInfo">${text[Math.floor(Math.random()*text.length)]}</p>`
  } else {
    let tempText = text.slice(0, num)
    //console.log(tempText) -> array

    let info = tempText.map(paragraphInfo => {
      return `<p class="textInfo">${paragraphInfo}</p> <br>`
    }).join("")
